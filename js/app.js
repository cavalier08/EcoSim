class Organism {
  ...
  hunt(ecosystem) {
    let potentialFood = ecosystem.filter((org) =>
      this.foodSource.includes(org.name)
    );
    ...
    for (let food of potentialFood) {
      let attempts = 0;
      while (
        this.currentFoodCount <= this.lowFoodLimit * this.count &&
        food.count > 0 &&
        attempts < MAX_HUNT_ATTEMPTS
      ) {
...
const organismsData = [
  ...
  {
    name: "Sparrow",
    args: [0.25, 10, 50, 0.5, 0.6, ["Grasshopper", "Wildflower"], 1],
  },
  ...
  { name: "Ant", args: [0.2, 0, 20, 0.03, 0.6, ["Tree", "Grass"], 0] },
  {
    ...
    args: [
      ...
      2,
      0.5,
      ["Rabbit", "Deer", "Sparrow", "Grass", "Wildflower", "Tree", "Shrub"],
      0,
    ],
  },
];
...
organismsData.forEach((orgData) => {
  ...
  initialCounts[orgData.name] =
    count === null || isNaN(count) || count < 0 ? 0 : parseInt(count);
});
...
const events = [
  {
    ...
    message:
      "A drought has occurred! Droughts severely affect the availability of water, which impacts both plants and animals. Without enough water, plants may not grow as fast or even die, leading to a decrease in food availability for herbivores like rabbits. As you'll see, this can cascade and impact predators like foxes too.",
  },
  {
    name: "Disease Outbreak",
    duration: 2, // Lasts for 2 days
    effect: (ecosystem) => {
      let affectedOrganism =
        ecosystem[Math.floor(Math.random() * ecosystem.length)];
      const affectedCount = Math.floor(affectedOrganism.count * 0.4); // 40% gets affected
      affectedOrganism.count -= affectedCount;
      alert(
        `Disease has affected the ${affectedOrganism.name}s! ${affectedCount} have died.`
      );
    },
    endEffect: (ecosystem) => {
    ...
    message:
      "A disease outbreak has occurred! Diseases can drastically reduce the population of affected species.",
  },
  {
    ...
    message:
      "A predator invasion has occurred! An influx of predators can lead to a change in predator-prey dynamics. Predators like foxes and coyote may hunt more frequently during this period.",
  },
  {
    name: "Bumper Crop",
    duration: 2,
    ...
    message:
      "A bumper crop has occurred! This event leads to an abundance of plants like grass, providing more food for herbivores. Herbivore populations may thrive during this period.",
  },
];
...
function ecosystemIntroduction() {
  addMessage(
    "Welcome to the ecosystem simulation! In this virtual environment, we'll observe the interactions between rabbits, foxes, grass, and weeds. Let's see how they co-exist and influence each other's populations."
  );
}
...
function checkFoodChainBroken() {
  const messages = {
    Grass:
      "The grass has gone extinct! Grass is a primary producer and forms the base of the food chain. Without it, herbivores will struggle for food and this can cause a chain reaction affecting the entire ecosystem.",
    Rabbit:
      "Oh no, the rabbits are extinct! They play a vital role in controlling plant populations and serving as prey for many predators. Their absence will ripple through the ecosystem.",
    ...
    Coyote:
      "The loss of coyotes, versatile predators, means their prey might grow unchecked, disturbing our delicate balance.",
    Bobcat:
      "Without bobcats, their primary prey, rabbits, may become overabundant, leading to vegetation stress.",
    Sparrow:
      "Sparrows have vanished! Insects they controlled might become a problem, and the predators that relied on them will suffer.",
    ...
    Wildflower:
      "Wildflowers are not just pretty to look at. Their extinction could affect various insects and the animals relying on those insects.",
    Shrub:
      "Shrubs are more than just plants; they are habitat and food for numerous organisms. Their absence will surely be felt.",
    ...
    Butterfly:
      "The delicate butterflies are gone! They played a role in pollination and were food for many; their loss will be noticed.",
    Beetle:
      "No more beetles? This might lead to an overgrowth of plants they consumed and a lack of food for those who ate them.",
    Grasshopper:
      "Grasshoppers, a key food source for many, are now extinct. Birds and other predators will definitely feel their absence.",
    ...
    Human:
      "Humans have vanished! While many ecosystems were significantly impacted by human activities, nature has a way of adapting and evolving. The absence of humans could lead to various changes, some of which might be recovery and rebalancing of certain ecosystems.",
  };

  const herbivores = ["Rabbit", "Deer", "Grasshopper"];
  if (
    herbivores.every((herb) =>
      ecosystem.some((org) => org.name === herb && org.count <= 0)
    )
  ) {
    alert(
      "Several herbivores are now extinct, which might lead to an overgrowth of plants. This could result in potential ecological imbalances."
    );
    return true;
  }
  ...
  if (
    apexPredators.every((predator) =>
      ecosystem.some((org) => org.name === predator && org.count <= 0)
    )
  ) {
    alert(
      "With the absence of apex predators, the animals they hunted might grow in numbers unchecked, leading to imbalances."
    );
    return true;
  }
  ...
  if (
    pollinators.every((pollinator) =>
      ecosystem.some((org) => org.name === pollinator && org.count <= 0)
    )
  ) {
    alert(
      "We're facing a potential pollination crisis with the extinction of key pollinators. This could threaten the reproduction of many plants."
    );
    return true;
  }
  ...
  if (
    primaryConsumers.every((consumer) =>
      ecosystem.some((org) => org.name === consumer && org.count <= 0)
    )
  ) {
    alert(
      "With many primary consumers gone, the predators will face a scarcity of prey. This might starve out many carnivores."
    );
    return true;
  }
  ...
  if (
    ecosystem.every(
      (org) => org.name !== "Grass" && org.name !== "Weed" && org.count <= 0
    )
  ) {
    alert(
      "It's a ghost town here! Almost every animal species has gone extinct. With only a few plants left, the ecosystem is on the brink of collapse."
    );
    return true;
  }
  return false;
...
function addMessage(message) {
  ...
  textBox.value += `Day ${dayCount + 1}. ${message}\n`;
...
function runSimulationDayByDay() {
  if (!checkFoodChainBroken()) {
    ...
    addMessage(`The food chain is broken on month ${dayCount}!`);
  }
}
